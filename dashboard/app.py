"""
Dashboard Streamlit - Benin Insights Challenge
...
# --- Config ------------------------------------------------------------------
st.set_page_config(
    page_title="Benin Insights 2025",
    page_icon="BJ",
    layout="wide",
...
# --- Palette coherente avec le notebook --------------------------------------
...
QUAD_MAP = {
    1: 'Cooperation verbale',
    2: 'Cooperation materielle',
    3: 'Conflit verbal',
    4: 'Conflit materiel'
}
...
# --- Chargement --------------------------------------------------------------
with st.spinner("Connexion a BigQuery en cours..."):
    df = load_events()
...
selected_months = st.sidebar.multiselect(
    ...
    help="Selectionner un ou plusieurs mois"
)

cats = ['Toutes'] + sorted(df['_category'].dropna().unique().tolist())
selected_cat = st.sidebar.selectbox("Type d'evenement", cats)
...
if 'AvgTone' in dff.columns:
    ...
    if pd.notna(avg_tone):
        k2.metric(
            "Ton moyen",
            f"{avg_tone:.2f}",
            delta="Positif" if avg_tone > 0 else "Negatif",
...
if 'GoldsteinScale' in dff.columns:
    ...
    if pd.notna(avg_gold):
        k3.metric(
            "Goldstein moyen",
            f"{avg_gold:.2f}",
            delta="Stabilisant" if avg_gold >= 0 else "Destabilisant",
...
if 'interaction_type' in dff.columns:
    ...
    k4.metric("Ratio Coop/Conflit", f"{ratio:.1f}x",
              help="Nombre d'evenements cooperatifs pour 1 conflit")
...
# --- VIZ 1 : Timeline + Repartition -----------------------------------------
st.subheader("Volume d'evenements par mois")
...
with col_a:
    ...
    fig.update_layout(
        ...
        xaxis_title="Mois", yaxis_title="Evenements"
    )
...
with col_b:
    if 'interaction_type' in dff.columns:
        ...
        fig_donut.add_annotation(
            text=f"<b>{total_donut:,}</b><br>evts",
            x=0.5, y=0.5, showarrow=False, font=dict(size=11)
        )
        st.plotly_chart(fig_donut, use_container_width=True)

# --- VIZ 2 : Ton + Goldstein -------------------------------------------------
st.subheader("Evolution du ton mediatique et de la stabilite")
...
fig3 = make_subplots(rows=1, cols=2,
    subplot_titles=['Ton mediatique moyen (AvgTone)', 'Score de stabilite (Goldstein)'])
fig3.add_trace(
...
# --- VIZ 3 : Cooperation vs Conflit ------------------------------------------
st.subheader("Cooperation vs Conflit - Evolution mensuelle")
...
if 'interaction_type' in dff.columns:
    ...
    fig_coop = px.line(
        ...
        markers=True,
        color_discrete_map={'Cooperation': COLOR_COOP_DARK, 'Conflict': COLOR_CONF_DARK},
        labels={'_month': 'Mois', 'n': 'Evenements', 'interaction_type': "Type d'interaction"},
        height=320
    )
    ...
    st.caption("La cooperation domine chaque mois (ratio min 2:1). Decembre : explosion simultanee des deux dynamiques.")

# --- VIZ 4 : Carte -----------------------------------------------------------
st.subheader("Carte des evenements au Benin")
...
if len(geo) > 0:
    ...
    st.caption(
        f"{len(geo):,} evenements geoloalises dans la bounding box Benin. "
        "Note : ~93% des evenements GDELT sont au centroide national (9.5N, 2.25E) - "
        "filtrez avec zoom pour voir les villes precises."
    )
else:
    st.info("Aucun evenement geolocalise avec les filtres actuels.")

# --- VIZ 5 : Top acteurs -----------------------------------------------------
st.subheader("Acteurs les plus mentionnes")
...
with col_d:
    if '_category' in dff.columns and '_sentiment' in dff.columns:
        ...
        fig6 = px.imshow(
            ...
            title="Sentiment par type d'evenement",
            text_auto=True, aspect='auto', height=420
        )
        fig6.update_layout(margin=dict(t=40))
        st.plotly_chart(fig6, use_container_width=True)

# --- VIZ 6 : Categories d'evenements -----------------------------------------
st.subheader("Categories d'evenements GDELT")
...
if 'EventCategory' in dff.columns:
    ...
    fig_cat = px.bar(
        ...
        labels={'nb_occurrences': "Nombre d'evenements", 'EventCategory': ''},
    ...
    fig_cat.update_traces(
        hovertemplate="<b>%{y}</b><br>Evenements : %{x:,}<extra></extra>"
    )
    st.plotly_chart(fig_cat, use_container_width=True)
    st.caption("La communication diplomatique et les consultations dominent (~54% des evenements).")

# --- VIZ 7 : Themes GKG ------------------------------------------------------
st.subheader("Themes dominants dans la presse mondiale (GKG)")

with st.expander("Charger les themes GDELT GKG (requete supplementaire)"):
        ...
        with st.spinner("Chargement gkg_clean..."):
            ...
            if col_themes:
                ...
                fig7 = px.bar(
                    ...
                    title='Top 20 themes GDELT lies au Benin',
                ...
                st.warning("Colonne V2Themes non trouvee dans gkg_clean.")
            ...
            if 'tone_category' in gkg.columns:
                ...
                fig_td = px.pie(
                    ...
                    title="Distribution du ton mediatique (GKG)",
                    color='tone_category', color_discrete_map=color_map_tone
                )
                fig_td.update_layout(margin=dict(t=40))
                st.plotly_chart(fig_td, use_container_width=True)
                st.caption("~59% des articles ont un ton negatif ou tres negatif - biais de negativite mediatique structurel.")

# --- VIZ 8 : Langues des sources ---------------------------------------------
st.subheader("Langues des sources mediatiques")

with st.expander("Charger la repartition linguistique (requete mentions)"):
    if st.button("Charger langues"):
        with st.spinner("Requete mentions_clean..."):
            try:
